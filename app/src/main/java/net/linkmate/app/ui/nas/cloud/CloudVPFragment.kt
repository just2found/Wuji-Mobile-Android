...
class CloudVPFragment : BaseNavFileFragment<OneOSFileType, OneOSFile>(),
    ...
    private val messageViewModel by viewModels<SystemMessageViewModel>()
            ...
            if (fragmentTags.isNotEmpty()) {
                ...
                if (dialogNoDevice == null) {
                    dialogNoDevice = DialogUtils.showNotifyDialog(requireContext(), null,
                            getString(R.string.tips_this_net_no_dev), getString(R.string.confirm))
                    { _, _ ->
                        activity?.finish()
                    }
                }
                    ...
                    if (!path.isNullOrEmpty()) {
                        ...
                        mAdapter?.currentPrimaryItem?.let { fragment ->
                            if (fragment.isResumed) {
                                if (fragment is QuickBaseCloudFragment) {
                                    Timber.d("share>>vp apply by setFileType path : $path")
                                    fragment.setFileType(type as OneOSFileType, path, DisplayMode.ALL)
                                }
                            } else {
