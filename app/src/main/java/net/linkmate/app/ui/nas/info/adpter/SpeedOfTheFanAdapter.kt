...
class SpeedOfTheFanAdapter() : BaseQuickAdapter<Int, BaseViewHolder>(R.layout.item_simple_list) {


    override fun convert(viewHolder: BaseViewHolder, p1: Int?) {
        viewHolder.setText(
            R.id.simple_title,
            " ${mContext.getString(R.string.fan_name)} ${viewHolder.adapterPosition+1} "
        )

        viewHolder.setText(
            R.id.simple_content,
            " $p1${mContext.getString(R.string.unit_of_speed)}"
        )
    }
