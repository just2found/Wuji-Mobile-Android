...
import net.linkmate.app.ui.nas.transfer.QuickTransmissionAdapter.Companion.getFailedInfo
...
class ViewHolder(val context: Context, val entity: TransferEntity) {
    ...
    private val transferStateObserver = TransferElement.TransferStateObserver {
        view.post { refreshUi() }
    }
...
//        subscribe()
    ...
    fun subscribe() {
        entity.subscribe(transferStateObserver)
    }

    fun clean() {
        entity.dispose(transferStateObserver)
    }
    ...
    fun refreshUi() {
        ...
        when (entity.state()) {
            ...
            TransferState.START -> {
                fileState.text = String.format("%s/s", FileUtils.fmtFileSpeed(abs(entity.getSpeed())))
            ...
            TransferState.FAILED -> {
                fileState.text = getFailedInfo(context, entity)
                positive.setText(R.string.transfer_continue)
            }
...
//        } else {
//            fileSize.text = String.format("%s/%s", FileUtils.fmtFileSize(entity.offset()),
