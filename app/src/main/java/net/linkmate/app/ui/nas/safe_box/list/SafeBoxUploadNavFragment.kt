 ...
 * Created by yun
 */
class SafeBoxUploadNavFragment : TipsBaseFragment() {

    private var mLastClickItem2Top: Int = 0
    private var mLastClickPosition: Int = -1

    private lateinit var mArgs: SafeBoxUploadNavFragmentArgs;
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            mArgs = SafeBoxUploadNavFragmentArgs.fromBundle(it)
        }
    ...
    override fun initView(view: View) {
        ...
        tv_path.setOnClickListener { v ->
            if (selectedFileCount == 0) {
                ...
                if (TextUtils.isEmpty(mToId)) {
                    ...
                    findNavController().navigate(R.id.action_upload_to_set_path, UploadSetPathFragmentArgs(mArgs.deviceid).toBundle())
                } else {
                    SessionManager.getInstance().getLoginSession(mToId!!, object : GetSessionListener() {
                        override fun onSuccess(url: String?, loginSession: LoginSession) {
                            ...
                            fileTreeView.setOnUploadListener(object : ServerFileTreeView.OnUploadFileListener {
                                override fun onUpload(toId: String?, tarPath: String?, type: Int) {
                                    mToId = toId
        ...
        fileAdapter = QuickSectionOneOSFilesRVAdapter(recycle_view.context, mFileList,
                mSelectedList)
        ...
        fileAdapter.onItemChildClickListener = BaseQuickAdapter.OnItemChildClickListener { adapter, view, position ->
    ...
    private fun getFileList(dir: File?) {
        ...
        if (!TextUtils.isEmpty(mSearchFilter)) {
            if (mFileType == LocalFileType.PRIVATE) {
                val task = LocalSortTask(requireContext(),this, mFileType, mSearchFilter, object : LocalSortTask.onLocalSortListener {
    ...
    fun changeFragmentByType(devId: String = mArgs.deviceid, fileType: LocalFileType = mSafeBoxFileListModel.fileType   , path: String = mArgs.path) {
        mFileType = fileType
        this.path = path
        this.mToId = devId
    ...
    companion object {
        private val TAG = SafeBoxUploadNavFragment::class.java.simpleName
    }
