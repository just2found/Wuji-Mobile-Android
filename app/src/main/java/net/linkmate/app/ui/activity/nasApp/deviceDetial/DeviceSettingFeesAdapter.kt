 ...
 * describe：
 */
class DeviceSettingFeesAdapter(val context: Context, val viewModel: DeviceSettingFeesViewModel) : RecyclerView.Adapter<DeviceSettingFeesAdapter.ViewHolder>() {
            ...
            DEFAULT_TYPE -> {
                ...
                with(holder.itemView) {
                    ...
                    fee?.let {
                        ...
                        //11.9需求 网络不显示，设备需显示积分单位
//                        if (!it.isFilterFlow())
                        if(it.isFilterFlow()){
                            val unit =  context.getString(R.string.fmt_traffic_unit_price2)
                                        .replace("\$TRAFFIC\$", MyConstants.DEFAULT_UNIT)
                            llCircleTypeForm.addView(getSubPanel(context.getString(R.string.required_points), it.getValueText()+" "+unit))
                        }else{
