...
import kotlinx.android.synthetic.main.activity_edit_circle_brief.*
import kotlinx.android.synthetic.main.activity_edit_circle_brief.etContent
import kotlinx.android.synthetic.main.activity_edit_circle_brief.fStatus
import kotlinx.android.synthetic.main.activity_edit_circle_brief.toolbar
 ...
 * describe：
 */
class EditCircleBriefActivity : BaseActivity() {
    private lateinit var mLoadingDialog: LoadingDialog
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_edit_circle_brief)
        initView()
    ...
    private fun initView() {
        ...
        data = BriefRepo.getBrief(deviceId, BriefRepo.FOR_CIRCLE)?.brief
        ...
        SessionManager.getInstance().getLoginSession(deviceId, object : GetSessionListener() {
            override fun onSuccess(url: String?, loginSession: LoginSession) {
                V5Repository.INSTANCE().setBrief(deviceId, loginSession.ip, LoginTokenUtil.getToken(),
                        BriefRepo.BRIEF_TYPE, BriefRepo.FOR_CIRCLE, content)
                        ...
                        .subscribe(object : V5Observer<BriefTimeStamp>(deviceId) {
                            ...
                            override fun success(result: BaseProtocol<BriefTimeStamp>) {
                                mLoadingDialog.dismiss()
                                if (result.result) {
                                    BriefRepo.insertAsync(deviceId, BriefRepo.FOR_CIRCLE, BriefRepo.BRIEF_TYPE, content, result.data?.update_at?.text, TxCallback { result, error ->
