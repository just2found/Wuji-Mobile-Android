...
class SafeBoxSetPathFragment : AbsChoiceFileFragment2() {

    private lateinit var mArgs: SafeBoxSetPathFragmentArgs;
    ...
    override fun onEnsureClick(sharePathType: Int, path: String): Boolean {
        ...
        mSafeBoxFileListModel.moveInSafeBox(mArgs.deviceid, path, mArgs.shareType, list).observe(this, Observer {
            if (it.status == Status.SUCCESS) {
                ToastUtils.showToast(R.string.move_file_success)
                findNavController().navigate(R.id.action_set_to_index, SafeBoxQuickCloudNavFragmentArgs(mArgs.deviceid).toBundle())
            } else {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            mArgs = SafeBoxSetPathFragmentArgs.fromBundle(it)
        }
    }
}