...
class SafeBoxNasFileAdapter(private val listLayoutID: Int = R.layout.item_choice_file,
                            private val gridLayoutID: Int = R.layout.item_choice_file_grid) : BaseQuickAdapter<OneOSFile, BaseViewHolder>(R.layout.item_choice_file) {
    /**
     * 下面是对外暴露的可以操作的
     */
    ...
    private fun localContainsFile(localFiles: List<File>, file: OneOSFile): Boolean {
        ...
        val toPath = SessionManager.getInstance().getDefaultDownloadPathByID(mLoginSession!!.id, file)
...
//        }
    ...
    private fun loadThumbByFresco(mLoginSession: LoginSession, imageView: ImageView, file: OneOSFile) {
        val url = OneOSAPIs.genThumbnailUrlV5(SharePathType.SAFE_BOX.type, mLoginSession, file.getPath(), "min")
