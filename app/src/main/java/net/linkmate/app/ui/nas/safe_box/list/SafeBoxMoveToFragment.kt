...
class SafeBoxMoveToFragment : AbsChoiceFileFragment2() {

    private lateinit var mArgs: SafeBoxMoveToFragmentArgs;
    ...
    override fun getNasFileInitData2(): NasFileInitData2 {
        return NasFileInitData2(
                ...
                mRootPathType= NasFileConstant.CONTAIN_USER or NasFileConstant.CONTAIN_PUBLIC or NasFileConstant.CONTAIN_SAFE
        )
    }
    ...
    override fun onEnsureClick(sharePathType: Int, path: String): Boolean {
        if (mArgs.type == MOVE) {
            liveData = mSafeBoxFileListModel.moveSafeBoxTo(mArgs.deviceid, path, sharePathType, pathList)
            ...
            liveData = mSafeBoxFileListModel.copySafeBoxTo(mArgs.deviceid, path, sharePathType, pathList)
            liveData.observe(this, Observer {
                if (it.status == Status.SUCCESS) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            mArgs = SafeBoxMoveToFragmentArgs.fromBundle(it)
