...
import net.linkmate.app.ui.nas.transfer_r.HeaderEntityR
...
import net.linkmate.app.ui.nas.transfer_r.ViewHolderR
...
class SbTransferFragment : TipsBaseFragment(), View.OnClickListener {

    private var isDownload: Boolean = false
    private val deviceViewModel by viewModels<DeviceViewModel>()
    private val devViewModel by viewModels<DevSelectViewModel>()
    private val transmissionViewModel by viewModels<SafeBoxTransmissionViewModel>() //传输数据的控制类
...
//                }
        ...
        transmissionViewModel.setRepositoryOperationParameter(devId
                ?: SafeBoxTransmissionViewModel.ALL_DEVICE, isDownload)
    ...
    private fun showTipToRemove(itemEntity: TransferEntityR, view: View, position: Int, callback: Callback<Boolean>) {
        val resIdTitle = if (itemEntity.isComplete) {
            R.string.confirm_clear_record
        } else if (itemEntity.isDownload) {
    ...
    companion object {
        fun newInstance(deviceId: String? = null): SbTransferFragment {
            val fragment = SbTransferFragment()
        ...
        val TAG: String = SbTransferFragment::class.java.simpleName
    }
