...
import net.linkmate.app.ui.nas.transfer_r.*
...
class SafeBoxTransmissionViewModel : ViewModel() {
    ...
    private fun wakeUp() {
        if (mIsDownLoad) {
            SafeBoxDownloadManager.wakeUpTask()
        } else {
            SafeBoxUploadManager.wakeUpTask()
        }
    }
    ...
    fun changeTransferStatus(entity: TransferEntityR, position: Int) {
        diskIOExecutor().execute {
            SafeBoxTransferHistoryKeeper.updateStateById(entity.id, entity.state)
        }
        if (entity.state == TransferState.WAIT || entity.state == TransferState.START) {
            ...
            if (mIsDownLoad) {
                SafeBoxDownloadManager.pause(entity.tag)
            } else {
                SafeBoxUploadManager.pause(entity.tag)
            }
        } else {
    ...
    fun changeTransferStatus(entity: HeaderEntityR) {
        ...
        if (entity.isAllStart) {
            ...
            if (mIsDownLoad) {
                SafeBoxDownloadManager.pauseDeviceId(mDeviceID)
            } else {
                SafeBoxUploadManager.pauseDeviceId(mDeviceID)
            }
        } else {
        ...
        diskIOExecutor().execute {
            if (flag) {
                SafeBoxTransferHistoryKeeper.updateStateByDevId(mDeviceID, TransferState.PAUSE, mIsDownLoad)
            } else {
                mProgressRepository.resume()
                SafeBoxTransferHistoryKeeper.updateStateByDevId(mDeviceID, TransferState.WAIT, mIsDownLoad)
                wakeUp()
            }
        }
    ...
    fun removeEntity(position: Int) {
        ...
        if (entity is TransferEntityR) {
            diskIOExecutor().execute {
                SafeBoxTransferHistoryKeeper.delete(entity.id)
                if (entity.isDownload) {
                    if (entity.state == TransferState.WAIT || entity.state == TransferState.START) {
                        SafeBoxDownloadManager.cancelTask(entity.tag, entity.id)
                    }
                    if (entity.state != TransferState.START) {
                        SafeBoxDownloadManager.notifyTransferReCount()
                    }
                } else {
                    SafeBoxUploadManager.notifyTransferReCount()
                    if (entity.state == TransferState.WAIT || entity.state == TransferState.START) {
                        SafeBoxUploadManager.cancelTask(entity.tag, entity.id)
                    }
                    if (entity.state != TransferState.START) {
                        SafeBoxUploadManager.notifyTransferReCount()
                    }
    ...
    fun deleteComplete() {
        diskIOExecutor().execute {
            SafeBoxTransferHistoryKeeper.deleteComplete(mDeviceID, mIsDownLoad)
    ...
    fun clearTransferFailed() {
        if (mProgressRepository.mDataSource.size <= 0)
            return
        diskIOExecutor().execute {
            val hasDeleteData = SafeBoxTransferHistoryKeeper.deleteByState(mDeviceID, TransferState.FAILED, mIsDownLoad)
            if (hasDeleteData) {
                if (mIsDownLoad) {
                    SafeBoxDownloadManager.notifyTransferReCount()
                } else {
    ...
    fun clearTransferProgress() {
        if (mProgressRepository.mDataSource.size <= 0)
            return
        diskIOExecutor().execute {
            if (SafeBoxTransferHistoryKeeper.deleteByRun(mDeviceID, mIsDownLoad)) {
                if (mIsDownLoad) {
                    SafeBoxDownloadManager.cancelDeviceId(mDeviceID)
                    SafeBoxDownloadManager.notifyTransferReCount()
                } else {
                    SafeBoxUploadManager.cancelDeviceId(mDeviceID)
                    SafeBoxUploadManager.notifyTransferReCount()
                }
                AppExecutors.instance.mainThread().execute {
                    reLoadData()
    ...
    fun setRepositoryOperationParameter(deviceID: String = mDeviceID, isDownLoad: Boolean = mIsDownLoad) {
        ...
        mProgressRepository.getRepositoryOperation()?.let {
            if (it is SbTransmissionRepositoryOperation) {
    ...
    private val mProgressRepository by lazy {
        TransferEntityRepository().apply {
            setRepositoryOperation(SbTransmissionRepositoryOperation(mDeviceID, mIsDownLoad, false))
    ...
    val mCompleteRepository by lazy {
        TransferEntityRepository().apply {
            setRepositoryOperation(SbTransmissionRepositoryOperation(mDeviceID, mIsDownLoad, true))
            setCallBack(object : CallBack<DataRefreshEvent> {
                override fun onCallBack(t: DataRefreshEvent) {
                    when (t.refreshType) {
                        NO_MORE_DATA -> {
...
//    }
    ...
    fun initTransmissionLiveData() {
        SafeBoxDownloadManager.setCallBack(mDownloadCallBack)
        SafeBoxUploadManager.setCallBack(mUploadCallBack)
    }
