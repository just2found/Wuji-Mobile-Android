...
public class QuotedPrintable {
            ...
            if (qp[i] == '=') {
                if (qplen - i > 2) {
                    if (qp[i + 1] == CR && qp[i + 2] == LF) {
                        ...
                        Logger.LOGI("asd", "decode: Invalid sequence = " + qp[i + 1] + qp[i + 2]);
                    }
                }
            }
     ...
     */
    public static String decode(@NonNull byte[] qp, @NonNull String enc) {
        int len = decode(qp);
        try {
            return new String(qp, 0, len, enc);
        } catch (UnsupportedEncodingException e) {
            Logger.LOGI("quote", "qp.decode: " + enc + " not supported. " + e.toString());
