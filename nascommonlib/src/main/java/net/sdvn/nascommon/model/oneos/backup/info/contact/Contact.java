...
public class Contact {
    ...
    private void initialize() {
        ...
        handleProp orgHandler = new handleProp() {
            ...
            @Override
            public void parseProp(@NonNull String propName, Vector<String> propVec, @NonNull String val) {
                String label = null;
        ...
        propHandlers.put("TEL", new handleProp() {
            public void parseProp(final String propName, @NonNull final Vector<String> propVec,
                                  final String val) {
                String label = null;
    ...
    String toCanonicalPhone(String phone) {
        ...
        // }
        // }
        // Logger.LOGI("Contact", "CanonicalPhone: " + phone);
        return phone;
    }
     ...
     * 从手机中获取通讯录
     */
    public void getContactInfoFromPhone(String id, @NonNull ContentResolver cResolver) {
        reset();
        peopleData._id = id;
        // 取出联系人姓名和_id
     ...
     * 将通讯录写入到vcf文件中
     */
    public void writeVCard(@NonNull Appendable vCardBuff) throws IOException {
     ...
     * @param org       单位信息
     */
    public static void formatOrg(@NonNull Appendable formatted, OrgData org) throws IOException {
        if (org.company != null) {
        ...
        if (imCur != null) {
            if (imCur.moveToFirst()) {
                do {
                    ...
                    Logger.LOGI("proNumber", proNumber);
                    if (proNumber != null) {
        ...
        try {
            if (postalCur != null && postalCur.moveToFirst()) {
                do {
                    ...
                    Logger.LOGI("postal", dataBuff + "  addr.num=" + addrs.size());
    ...
    private void getNoteFields(ContentResolver cResolver) {
        ...
        try {
            if (cursor != null && cursor.moveToFirst()) {
                ...
                Logger.p(Logger.Level.INFO, Logger.Logd.DEBUG, " notes ", new Gson().toJson(notes));
                if (notes != null) {
    ...
    private void getPhoneFields(ContentResolver cResolver) {
        ...
        Cursor cur = cResolver.query(Data.CONTENT_URI, projection, selection, selParams, null);
        if (cur != null) {
            Logger.LOGI("count = ", "" + cur.getCount());
            if (cur.moveToFirst()) {
                do {
                    String customType = null;
     ...
     * 追加到文件尾部
     */
    private static void appendField(@NonNull Appendable out, String name, @Nullable String val) throws IOException {
     ...
     * 解析vcard
     */
    public long parseVCard(@NonNull BufferedReader vCard) throws IOException {
        reset();
        // 读取BEGIN
     ...
     */
    private void RemoveContact(@NonNull ContentResolver cResolver, long key) {
        // 移除已有联系人
        if (key != 0) {
     ...
     * @return 插入的列所在行
     */
    public long addContact(@NonNull Context context, long key, boolean replace) {
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                Logger.LOGI("people.getString(0)", cursor.getString(0));
                if (replace) {
                    do {
                        setId(cursor.getString(1));
                        key = getId();
                        RemoveContact(cResolver, key);
                        Logger.LOGI("Contact", "OverwriteContact");
    ...
    public ContentValues getPeopleCV() {
        ...
        Logger.LOGI("peopleData.displayName", peopleData.displayName);
     ...
     */
    @Nullable
    public ContentValues getOrgnizeCV(@NonNull OrgData org) {
     ...
     */
    @NonNull
    public ContentValues getPhoneCV(@NonNull RowData data) {
        ContentValues cv = new ContentValues();
     ...
     */
    @NonNull
    public ContentValues getEmailCV(@NonNull RowData data) {
        ContentValues cv = new ContentValues();
     ...
     */
    @NonNull
    public ContentValues getAddressCV(@NonNull RowData data) {
        ContentValues cv = new ContentValues();
        // 类型
        ...
        if (addressFields[maxLen - 6] != null) {
            Logger.LOGI("getAddressCV", "no neighborhood");
        }
        // 邮箱
     ...
     */
    @NonNull
    public ContentValues getImCV(@NonNull RowData data) {
        ContentValues cv = new ContentValues();
     ...
     */
    @Nullable
    private Uri insertPeopleValue(@NonNull ContentResolver cResolver, @NonNull Uri uri, @Nullable ContentValues cv) {
        if (cv != null) {
