...
public class UdpScanDeviceTask extends AsyncTask<Void, String, String[]> {
    ...
    protected String[] doInBackground(Void... params) {
        ...
        try {
            ...
            while (!isInterrupt && (System.currentTimeMillis() - start < TIME_SCAN)) {
                ...
                final String ip = receivePacket.getAddress().getHostAddress();
                if (Utils.isAvailableIp(ip)) { // Mac: 84:5D:D7:02:00:0D ; Len: 17
                    String deviceMac = new String(receivePacket.getData()).substring(0, 17).toUpperCase();
                    Logger.LOGD(TAG, "Scanning: IP= " + ip + "  Mac:" + deviceMac);
    ...
    protected void onProgressUpdate(String... values) {
        ...
        // mIpText.setTitleText(values[1]);
        // }
        Logger.LOGD(TAG, "Store: IP= " + values[1] + "  Mac:" + values[0]);
    ...
    protected void onPostExecute(String[] result) {
        Logger.LOGD(TAG, "---on Post Execute---");
