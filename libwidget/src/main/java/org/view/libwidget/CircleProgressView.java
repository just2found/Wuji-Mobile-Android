...
public class CircleProgressView extends ProgressBar {
    ...
    private int mReachBarSize; // 未完成进度条大小
    private int mNormalBarSize; // 未完成进度条大小
    private int mTextSize; // 进度值字体大小
    ...
    private int mInnerPadding; // 内部圆与外部圆间距
    ...
    public CircleProgressView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mReachBarSize = dip2px(getContext(), 2); // 未完成进度条大小
        mNormalBarSize = dip2px(getContext(), 2);
        mTextSize = sp2px(getContext(), 14);
        mOuterSize = dip2px(getContext(), 1); // 外层圆环宽度
        mInnerPadding = dip2px(getContext(), 1);
        mRadius = dip2px(getContext(), 20);
        ...
        switch (mProgressStyle) {
            ...
            case ProgressStyle.NORMAL:
                ...
                if (ta.hasValue(R.styleable.CircleProgressView_cpv_innerBackgroundColor)) {
                    mInnerBackgroundColor = ta.getColor(R.styleable.CircleProgressView_cpv_innerBackgroundColor, Color.argb(88, 255, 255, 255));
    ...
    private void drawFillInArcCircle(Canvas canvas) {
        canvas.save();
        canvas.translate(mRealWidth / 2f, mRealHeight / 2f);
        // 绘制外层圆环
    ...
    private void drawNormalCircle(Canvas canvas) {
        ...
        // 绘制内部圆形背景色
        if (needDrawInnerBackground) {
            canvas.drawCircle(0, 0, mRadius - Math.min(mReachBarSize, mNormalBarSize) / 2f,
