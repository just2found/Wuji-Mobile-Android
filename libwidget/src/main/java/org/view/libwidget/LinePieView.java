...
public class LinePieView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //设置半径为宽高最小值的1/2
        radius = Math.min(getMeasuredWidth(), getMeasuredHeight()) / 2.5f;
        //设置扇形外接矩形
    ...
    private void calculateAndDraw(Canvas canvas) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            ...
            //float arcCenterDegree = startAngle - angle / 2;
     ...
     * @param colors  颜色数组
     */
    public void setData(int[] numbers, String centerText, int[] colors) {
        if (numbers == null || numbers.length == 0
     ...
     * @param numbers 数字数组
     */
    public void setData(int[] numbers, String centerText) {
        if (numbers == null || numbers.length == 0) {
            return;
        }
        this.numbers = numbers;
