...
abstract class LeoRvAdapter<T : Any> : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
     ...
     * @param data   对应数据
     */
    protected abstract fun bindData(helper: ItemHelper, data: T, payloads: MutableList<Any>? = null)
    ...
    inner class ViewHolder internal constructor(parent: ViewGroup, layout: Int) :
        ...
        fun onBindViewHolder(position: Int, payloads: MutableList<Any>? = null) {
            mPosition = position
            bindData(itemHelper, data[mPosition], payloads)
            loadMore(mPosition)
        }
    ...
    class ItemHelper(val viewHolder: LeoRvAdapter<out Any>.ViewHolder) : View.OnClickListener {
        ...
        fun setGone(@IdRes viewId: Int, isVisible: Boolean): ItemHelper {
            getViewById<View>(viewId) {
                it.visibility = if (isVisible) {
                    View.VISIBLE
                } else {
                    View.GONE
                }
        ...
        fun setImageDrawable(@IdRes viewId: Int, drawable: Drawable?): ItemHelper {
            getViewById<View>(viewId) {
                if (it is ImageView) {
    ...
    abstract class ItemHolder<T : Any> {
         ...
         * @param item   数据
         */
        abstract fun bindData(helper: ItemHelper, item: T, payloads: MutableList<Any>? = null)
