 ...
 *         <li>Default: {@code R.layout.player_control_view}
 *       </ul>
 ...
 * configurations, you can define {@code player_control_view.xml} layout files in your
 ...
 * <p>
 * Defining your own {@code player_control_view.xml} is useful to customize the layout of
 ...
 * {@code player_control_view.xml} for only the instance on which the attribute is set.
 */
public class PlayerControlView extends FrameLayout {
    ...
    @Nullable
    private final View lockOff;
    @Nullable
    private final View lockOn;
    @Nullable
    private final View back;
    @Nullable
    private final View backTop;
    @Nullable
    private final View fullScreenEnter;
    @Nullable
    private final View fullScreenExit;
    @Nullable
    private final View floatScreen;
    @Nullable
    ...
    private boolean showLockF;
    private boolean showBack;
    private boolean showBackTop;
    private boolean showFullScreenEnter;
    private boolean showFullScreenExit;
    private boolean showFloatScreen;
    ...
    public PlayerControlView(
        ...
        showBack = true;
        showBackTop = true;
        showFullScreenEnter = true;
        showFullScreenExit = false;
        showFloatScreen = true;
        ...
        if (playbackAttrs != null) {
            ...
            try {
                ...
                showRewindButton =
                        a.getBoolean(com.google.android.exoplayer2.ui.R.styleable.PlayerControlView_show_rewind_button, showRewindButton);
                showFastForwardButton =
                        a.getBoolean(
                                com.google.android.exoplayer2.ui.R.styleable.PlayerControlView_show_fastforward_button, showFastForwardButton);
                showPreviousButton =
                        a.getBoolean(com.google.android.exoplayer2.ui.R.styleable.PlayerControlView_show_previous_button, showPreviousButton);
                showNextButton =
                        a.getBoolean(com.google.android.exoplayer2.ui.R.styleable.PlayerControlView_show_next_button, showNextButton);
                showShuffleButton =
        ...
        backTop = findViewById(R.id.player_back_top);
        if (backTop != null) {
            Resources resources = context.getResources();
            int resourceId = resources.getIdentifier("status_bar_height", "dimen", "android");
            int height = resources.getDimensionPixelSize(resourceId);
            backTop.setPadding(0, height, 0, 0);
        }
        lockOff = findViewById(R.id.player_lock_f);
        if (lockOff != null) {
            lockOff.setOnClickListener(componentListener);
        }
        lockOn = findViewById(R.id.player_lock_n);
        if (lockOn != null) {
            lockOn.setOnClickListener(componentListener);
        }
        back = findViewById(R.id.player_back);
        if (back != null) {
            back.setOnClickListener(componentListener);
        }
        fullScreenEnter = findViewById(R.id.player_full_screen_enter);
        if (fullScreenEnter != null) {
            fullScreenEnter.setOnClickListener(componentListener);
        }
        fullScreenExit = findViewById(R.id.player_full_screen_exit);
        if (fullScreenExit != null) {
            fullScreenExit.setOnClickListener(componentListener);
        }
        floatScreen = findViewById(R.id.player_float_screen);
        if (floatScreen != null) {
            floatScreen.setOnClickListener(componentListener);
        }
    ...
    /**
     * Sets the {@link MyOnClickListener}.
     *
     * @param listener The listener to be notified about when click.
     */
    public void setOnClickListener(@Nullable MyOnClickListener listener) {
    ...
    /**
     * Sets whether the 锁 button is shown.
     *
     * @param showLock Whether the 锁 button is shown.
     */
    ...
    /**
     * Sets whether the 锁开 button is shown.
     *
     * @param showLockOff Whether the 锁开/锁关.
     */
    public void setShowLockOff(boolean showLockOff) {
        this.showLockF = showLockOff;
        updateNavigation();
    }

    public boolean getShowLockOff() {
        return showLockF;
    }

    /**
     * Sets whether the 返回 button is shown.
     *
     * @param showBack Whether the previous button is shown.
     */
    ...
    /**
     * Sets whether the 全屏 button is shown.
     *
     * @param show Whether the previous button is shown.
     */
    public void setShowFullScreenEnter(boolean show) {
        this.showFullScreenEnter = show;
        updateNavigation();
    }

    public void setShowFullScreenExit(boolean show) {
        this.showFullScreenExit = show;
        updateNavigation();
    }

    public void setShowPortrait(boolean isPortrait) {
        this.showBackTop = isPortrait;
        updateNavigation();
    }

    public void setShowFullScreen(boolean showFullScreen) {
        this.showFullScreenEnter = !showFullScreen;
        this.showFullScreenExit = showFullScreen;
        this.showFloatScreen = !showFullScreen;
        this.showBackTop = !showFullScreen;
        updateNavigation();
    }

    /**
     * Sets whether the 小窗 button is shown.
     *
     * @param showFloatScreen Whether the previous button is shown.
     */
    public void setShowFloatScreen(boolean showFloatScreen) {
        this.showFloatScreen = showFloatScreen;
        updateNavigation();
    }
    ...
    private void updatePlayPauseButton() {
        ...
        if (playButton != null) {
            ...
            playButton.setVisibility(showLock && !showLockF ? GONE : shouldShowPauseButton ? GONE : VISIBLE);
        }
        if (pauseButton != null) {
            requestPlayPauseFocus |= !shouldShowPauseButton && pauseButton.isFocused();
            pauseButton.setVisibility(showLock && !showLockF ? GONE : shouldShowPauseButton ? VISIBLE : GONE);
        }
    ...
    private void updateNavigation() {
        ...
        updateButton(showLock ? (showPreviousButton && showLockF) : showPreviousButton, enablePrevious, previousButton);
        updateButton(showLock ? (showRewindButton && showLockF) : showRewindButton, enableRewind, rewindButton);
        updateButton(showLock ? (showFastForwardButton && showLockF) : showFastForwardButton, enableFastForward, fastForwardButton);
        updateButton(showLock ? (showNextButton && showLockF) : showNextButton, enableNext, nextButton);
        updateButton(showLock && showLockF, true, lockOff);
        updateButton(showLock && !showLockF, true, lockOn);
        updateButton(showLock ? (showBack && showLockF) : showBack, true, back);
        updateButton(showLock ? (showBackTop && showLockF) : showBackTop, true, backTop);
        updateButton(showLock ? (showFullScreenEnter && showLockF) : showFullScreenEnter, enableSeeking, fullScreenEnter);
        updateButton(showLock ? (showFullScreenExit && showLockF) : showFullScreenExit, enableSeeking, fullScreenExit);
        updateButton(showLock ? (showFloatScreen && showLockF) : showFloatScreen, enableSeeking, floatScreen);
        updateButton(showLock && showLockF, enableSeeking, positionView);
        updateButton(showLock && showLockF, enableSeeking, durationView);
        if (timeBar != null) {
            timeBar.setEnabled((showLock && !showLockF) ? false : enableSeeking);
        }
    }
    ...
    private final class ComponentListener
        ...
        public void onClick(View view) {
            ...
            } else if (lockOff == view) {//解锁
                setShowLockOff(false);
                updatePlayPauseButton();
            } else if (lockOn == view) {//锁屏
                setShowLockOff(true);
                updatePlayPauseButton();
            } else if (player == null) {
                return;
            ...
            } else if (fullScreenEnter == view) {//全屏
                if (onClickListener != null) onClickListener.onClick(view);
            } else if (fullScreenExit == view) {//退出全屏
                if (onClickListener != null) onClickListener.onClick(view);
            } else if (floatScreen == view) {//小窗播放
                if (onClickListener != null) onClickListener.onClick(view);
            }
        }
    }
