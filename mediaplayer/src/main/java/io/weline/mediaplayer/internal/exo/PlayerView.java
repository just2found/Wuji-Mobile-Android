...
import io.weline.mediaplayer.internal.view.BrightnessDialog;
import io.weline.mediaplayer.internal.view.SeekDialog;
import io.weline.mediaplayer.internal.view.VolumeDialog;
 ...
 *       inflated by the child {@link
 *       PlayerControlView}. See below for more details.
 *       <ul>
 *         <li>Corresponding method: None
 *         <li>Default: {@code R.layout.exo_player_control_view}
 *       </ul>
 *   <li>All attributes that can be set on {@link com.google.android.exoplayer2.ui.PlayerControlView} and {@link DefaultTimeBar} can
 ...
 * <p>
 * The drawables used by {@link
 * PlayerControlView} (with its default layout file) can be overridden
 * by drawables with the same names defined in your application. See the {@link com.google.android.exoplayer2.ui.PlayerControlView}
 ...
 *       {@link
 *       PlayerControlView}. Ignored if an {@code exo_controller} view exists.
 *       <ul>
 *         <li>Type: {@link View}
 *       </ul>
 *   <li><b>{@code exo_controller}</b> - An already inflated {@link
 *   PlayerControlView}. Allows use
 *       of a custom extension of {@link
 *       PlayerControlView}. {@link
 *       PlayerControlView} and {@link
 ...
 */
public class PlayerView extends FrameLayout implements AdsLoader.AdViewProvider/*, View.OnClickListener*/ {
    ...
    @Nullable
    private PlayerControlView.MyOnClickListener onClickListener;
...
//      surfaceView.setOnClickListener(this);
    ...
    /**
     * Set the {@link PlayerControlView.MyOnClickListener}.
     *
     * @param listener The listener to be notified about visibility changes, or null to remove the
     *                 current listener.
     */
    public void setOnClickListener(
            @Nullable PlayerControlView.MyOnClickListener listener) {
        Assertions.checkStateNotNull(controller);
        if (this.onClickListener == listener) {
            return;
        }
     ...
     * @param show
     */
    public void setShowFloatScreen(boolean show) {
        Assertions.checkStateNotNull(controller);
        controller.setShowFloatScreen(show);
    }
    ...
    public void setShowFullScreenEnter(boolean show) {
        Assertions.checkStateNotNull(controller);
        controller.setShowFullScreenEnter(show);
    }

    public void setShowFullScreenExit(boolean show) {
        Assertions.checkStateNotNull(controller);
        controller.setShowFullScreenExit(show);
    }

    public void setShowPortrait(boolean isPortrait) {
        Assertions.checkStateNotNull(controller);
        controller.setShowPortrait(isPortrait);
    }
    ...
    public void setShowLock(boolean showLock) {
        Assertions.checkStateNotNull(controller);
        controller.setShowLock(showLock);
    }

    public boolean isLocked() {
        Assertions.checkStateNotNull(controller);
        return controller.getShowLock() && !controller.getShowLockOff();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (touchListener == null) {
                    //小窗onTouch监听时放弃手势控制音量等
                    downVolume = getStreamVolume();
                    downBrightness = getScreenBrightness(getContext());
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (touchListener == null) {
                    //小窗onTouch监听时放弃手势控制音量等
                    touchMove(mStopX - mStartX, mStopY - mStartY, event.getX());
                }
                return true;
            case MotionEvent.ACTION_UP:
                mStopX = (int) event.getX();
                mStopY = (int) event.getY();
                if (isTouching && Math.abs(mStartX - mStopX) <= 1 || Math.abs(mStartY - mStopY) <= 1) {
                    isTouching = false;
                    performClick();
                }
                if (touchListener == null) {
    ...
    public void setSupportGesture(boolean isSupportVolume, boolean isSupportBrightness, boolean isSupportSeek) {
        this.isSupportVolume = isSupportVolume;
    ...
    protected void changeProgress(float dx) {
        int distance = getWidth();
        assert player != null;
        long videoDuration = player.getContentDuration();
        ...
        String progressText = stringForTime(newVideoPosition, false) + "/" + stringForTime(videoDuration, false);
    ...
    protected void changeBrightness(float dy) {
        ...
        setScreenBrightness(getContext(), newBrightness);
